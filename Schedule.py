...
def main():
    ...
    selected_robot_value = selected_robot.selectbox("Select a Robot", robot_options)
    ...
    start_date_value = start_date.date_input("Start Date", default_start_date)
    end_date_value = end_date.date_input("End Date", default_end_date)

    # Display the selected robot and date range with red color
    #st.markdown(f"You selected: <span style='color:red'>{selected_robot_value}</span>, "
               # f"Start Date: {start_date_value}, End Date: {end_date_value}", unsafe_allow_html=True)
...
if __name__ == "__main__":
    ...
    styled_df2 = df2.style.hide(axis="index").set_table_styles([
        {'selector': '', 'props': [('background-color', '#7b7f89'), ('color', 'black')]},
        {'selector': 'th', 'props': [('background-color', '#7b7f89'), ('color', 'black')]},
    ])
